    ...
    for f in os.scandir(in_dir):
        if f.name.endswith(".bingeom"):
            ...
            r.success, r.log = _compare_binary_files(
                filepath1=f.path,
                filepath2=os.path.join(test.path, ref_dir, f.name),
            )
            r.time = time.time() - start_time
            r.print()
...
def exec_tests(options):
    """!
    Execute tests in TEST_DIR.
    """
    for test in os.scandir(path=TEST_DIR):
        if (
            test.is_dir()
            and test.name.startswith("test")
            and os.path.isfile(f"{test.path}/run.py")
            and not os.path.isfile(f"{test.path}/skip")
            and (not options or (test.name in options))
        ):
            print(f"\n{bcolors.HEADER}Exec test <{test.name}>{bcolors.ENDC}")
            exec(
                open(f"{test.path}/run.py").read(),
                globals(),
                {"test": test},  # send dir
            )
    if not options:
...
if __name__ == "__main__":
    ...
    try:
        import bpy
    except ModuleNotFoundError:  # Blender not running
        first_run = True
    else:
        first_run = False  # Blender running
    if first_run:
        ...
        try:
            ...
            raise ModuleNotFoundError("BlenderFDS addon not loaded.")
        exec_tests(options=sys.argv[5:])
